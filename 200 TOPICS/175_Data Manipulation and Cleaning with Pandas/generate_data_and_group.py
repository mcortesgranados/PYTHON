...
# Method to group by gender orientation and calculate count and sum
def group_by_orientation(df):
    # Group by gender orientation
    grouped = df.groupby('Orientation')
    
    # Calculate count and sum for each gender orientation
    summary = grouped.agg({'Value': ['count', 'sum']})
    
    # Format sum column with thousands separator
    summary = summary.applymap(lambda x: f'{x:,}')
    
    return summary

# Call the method
result = group_by_orientation(df)
print(result)
